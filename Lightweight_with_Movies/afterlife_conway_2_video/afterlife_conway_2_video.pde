...
void draw() {
  ...
  init_img = createImage(n_inputs, n_outputs, RGB);
  if(mov.available()) {
    mov.read();
  }
  //input_rectangle.img.copy(mov, 0, 0, 1280, 720, 0, 0, 640, 480);
  init_img.copy(mov, 0, 0, 1920, 1080, 0, 0, n_inputs, n_outputs); 
  init_img.filter(GRAY);
  //init_img.filter(INVERT);
  //init_img.resize(n_inputs, n_outputs);
  for (int i = 0; i < n_inputs; i++)
    for (int j = 0; j < n_outputs; j++) {
      outputs[1].matrix[i][j] = 10*(brightness(init_img.get(i,j))-1.0); // with cheating 10*
    } 
  ...
  for (int i = 0; i < n_inputs; i++)
    for (int j = 0; j < n_outputs; j++) {
       my_max=max(my_max,outputs[1].matrix[i][j]);   
       my_min=min(my_min,outputs[1].matrix[i][j]);
    }
    
  println(my_min, my_max);  
  //outputs[1].squeeze();
